...
def _get_confirmed_time(state: SecurityState, chart_time: int) -> int:
    ...
    For same timeframe: a chart bar and its same-time security bar close at the
    same instant, so the target is the chart bar's own open — clamped to the
    latest real child bar open when ``bar_opens`` is loaded (a gappy cross-symbol
    feed, e.g. a session-bounded bond yield on a 24/7 crypto chart). Between real
    bars nothing new is confirmed, so ``gaps_off`` forward-fills the last real
    value and ``gaps_on`` emits ``na`` — TradingView never evaluates the
    expression on the writer's synthetic gap-fill bars.
