...
class CompilationService:
    ...
    def __init__(self, api_client: PynesysAPIClient | None = None, config: APIConfig | None = None):
        """
        Initialize the compilation service.

        :param api_client: Optional pre-configured API client
        :param config: Optional API configuration (will load from default if not provided)
        """
        if api_client:
    ...
    def compile_file(
        ...
        if pine_file_path.suffix != '.pine':
            raise ValueError(
                f"This file format isn't supported: {pine_file_path.suffix}. "
                f"Only .pine files can be compiled! ✨ Try using a .pine file instead.")
...
def create_compilation_service(
    ...
    :return: Configured CompilationService instance
