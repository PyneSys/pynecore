...
class BrokenInvariantProfile(ReferenceVenueProfile):
    ...
    def handle_step(self, runner, step):
        if step.kind == "resurrect_terminal":
            ...
            record.order = replace(record.order, status=OrderStatus.OPEN, remaining_qty=1.0)
            return True
        ...
        if step.kind == "venue_position_drift":
            self.state.positions[step.run] = 1.0
            return True
        return super().handle_step(runner, step)
...
SMOKE = (
    ...
    Scenario(
        ...
        seed=0,
        expected_violation="protection coverage shortfall",
        steps=(
            Step("entry", values={"id": "L", "qty": 2.0}),
            Step("sync"),
            Step("fill", check_invariants=False),
    ...
    Scenario(
        ...
        profile_factory=BrokenInvariantProfile,
        seed=0,
        expected_violation="economic position mismatch",
...
def _extended(seed: int) -> list[Scenario]:
    ...
    for index, case in enumerate(pairwise_cases(axes, seed=seed)):
        qty = 2.0 if case["fill"] == "partial" or case["ordering"] == "reordered" else 1.0
        ...
        if case["restart"]:
            steps.extend((Step("restart"), Step("entry", values=order_values), Step("sync")))
        steps.append(
            Step(
                "fill",
        ...
        if case["ordering"] == "delayed":
            steps.extend(
                (
                    Step("delayed_deliver", values={"delay_ms": 10}, check_invariants=False),
        ...
        scenarios.append(
            Scenario(
                ...
                profile_factory=(HedgedReferenceVenueProfile if case["mode"] == "hedged" else ReferenceVenueProfile),
                steps=tuple(steps),
                runs=runs,
                seed=seed,
