...
holiday calendar and confirms the daily bar on that day's LAST chart bar.

PyneCore confirms on SCHEDULED closes only. An unscheduled early close is
indistinguishable from a data gap, and a bar-grid rule ("the next chart bar is
already in a later period, so this one ends the period") reads the future on a
gappy feed — the one thing that is never traded away. The half-day's daily bar
therefore arrives one chart bar late, on the first bar whose own close passes
the scheduled 16:00. MEASURED on ``AMEX:SPY@5`` requesting ``"1D"``: 2025-11-28
and 2025-12-24 are the only two bars out of 20207 this defers, and live mode
already behaved exactly this way.
"""
...
def _daily_state(opens: list[int]) -> SecurityState:
    ...
    state.bar_closes = [
        actual_bar_close(o, 0, BarCalendar(tz=_NY, opening_hours=tuple(hours),
                                           session_starts=tuple(starts)), "1D")
        for o in opens
    ]
...
def __test_early_close_defers_by_one_chart_bar__(log):
    """A 13:00 half-day is NOT in the schedule, so its daily bar arrives one bar late.

    Confirming it on the 12:55 bar would mean reading the bar grid for a
    calendar the schedule does not carry — and the same rule hands a gappy feed
    a full future bar. The daily value lands on the next session's first chart
    bar instead, which is what live mode has always done.
    """
    ...
    assert got[1] < d28          # the scheduled 16:00 close has not been reached
    assert got[2] == d28         # the next session's first bar passes it
