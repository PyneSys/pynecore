...
LOWS = [100.0 - bar for bar in range(12)] + [200.0 + bar for bar in range(24)]
...
def _rows():
    ...
    return [OHLCV(timestamp=base_ts + bar * 1800, open=lo + 5.0, high=lo + 10.0, low=lo,
            ...
            for bar, lo in enumerate(LOWS)]


def _expected_gated() -> list[float]:
    """The measured machine, spelled out independently of the implementation."""
    window = [float("nan")] * CAPACITY
    out: list[float] = []
    for bar, lo in enumerate(LOWS):
        if bar in SKIPPED:
            out.append(-1.0)
            continue
        window[bar % CAPACITY] = lo
        if bar < LENGTH - 1:
            out.append(float("nan"))
            continue
        # ``avail`` counts bars since the site's first call, capped at the window
        seen = min(bar + 1, LENGTH)
        out.append(min(window[(bar - i) % CAPACITY] for i in range(seen)))
    return out
...
def __test_ungated_lowest_is_the_plain_window__(runner):
    ...
    for bar, (_candle, plot) in enumerate(runner(iter(_rows())).run_iter()):
        ...
        if bar < LENGTH - 1:
            assert value != value, f"bar {bar}: expected na, got {value}"
            continue
        assert value == min(LOWS[bar - LENGTH + 1:bar + 1]), f"bar {bar}: {value}"
