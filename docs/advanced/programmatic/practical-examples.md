...
This page demonstrates real-world applications of PyneCore's programmatic API. These examples show how to build automation systems, batch processing workflows, and trading applications using Python code instead of CLI commands.
...
def manual_csv_conversion(csv_path: Path, symbol: str, timezone: str = "UTC"):
...
manual_csv_conversion(Path("custom_data.csv"), "CUSTOM")
```
...
These practical examples demonstrate how to achieve programmatically everything that's available through the CLI, plus additional automation and integration capabilities that are only possible through the Python API.